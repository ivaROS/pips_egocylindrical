...
namespace pips_egocylindrical
{
  ...
  PipsEgocylindricalTrajectoryController::PipsEgocylindricalTrajectoryController(ros::NodeHandle& nh, ros::NodeHandle& pnh) :
    ObstacleAvoidanceController(nh, pnh), 
    nh_(nh, name_), 
    pnh_(pnh, name_)
  {
    cc_ = std::make_shared<pips::collision_testing::EgocylindricalImageCollisionChecker>(nh_, pnh_);
  ...
  void PipsEgocylindricalTrajectoryController::ecImageCb(const sensor_msgs::Image::ConstPtr& image_msg,
  ...
  bool PipsEgocylindricalTrajectoryController::init()
  {
    ...
    std::string depth_image_topic = "/range_image";
    std::string depth_info_topic = "/egocylindrical_points";
    
    ROS_DEBUG_STREAM_NAMED(name_,  "Setting up publishers and subscribers");
    
    ec_sub_.subscribe(ObstacleAvoidanceController::nh_, depth_image_topic, 1);
    ec_info_sub_.subscribe(ObstacleAvoidanceController::nh_, depth_info_topic, 1);
    synced_images.reset(new image_synchronizer(image_synchronizer(10), ec_sub_, ec_info_sub_) );
    synced_images->registerCallback(bind(&PipsEgocylindricalTrajectoryController::ecImageCb, this, _1, _2));
